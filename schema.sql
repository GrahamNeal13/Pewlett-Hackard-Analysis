...
--New table for current employees eligible for retirement.
SELECT ri.emp_no,
    ri.first_name,
    ri.last_name,
de.to_date
...
GROUP BY de.dept_no
--Add Order By to organize the output
ORDER BY de.dept_no;

--Create csv file of the employee count by department number table
SELECT COUNT (ce.emp_no), de.dept_no
INTO retire_by_dept
FROM current_emp as ce
LEFT JOIN dept_emp as de
